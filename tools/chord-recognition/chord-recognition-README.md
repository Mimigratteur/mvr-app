...
## Port JavaScript integre directement dans MVR (31/07/2026, soir)

L'algorithme (recognize_windowed avec cle Krumhansl-Kessler, window_s=1.0
valide) a ete porte en JavaScript pur (aucune dependance externe) et
integre directement dans index.html de MVR - fonctionne dans le
navigateur, plus besoin de Python ni d'outil separe. Bouton "Analyser un
fichier audio" dans l'onglet Importer, reutilise l'ecran de relecture de
l'import PDF (memes champs) pour que l'utilisateur corrige le brouillon
avant de creer le morceau.

Valide de bout en bout dans un vrai navigateur (Playwright/Chromium) :
- Syntaxe verifiee, aucune fonction dupliquee
- Precision revalidee sur les 3 memes references Python (Emmenez-moi
  MIDI, La Corrida, Gary Moore) : 35% exact / 41% fondamentale au total -
  coherent avec les chiffres Python (recognize_windowed seul, sans le
  Viterbi ni les ameliorations ulterieures qui restent Python uniquement
  pour l'instant)
- Testee sur fichier MP3 complet (5:38) : 6.2s d'analyse, pas de blocage
- Testee sur viewport mobile (390x844) : fonctionne
- Testee sur morceau rock distordu : meme limite deja documentee
  (grille bruitee), pas de crash

Fenetre par defaut passee de 2.0s (Python) a 1.0s en JS : un bug reel a
ete trouve en testant sur Emmenez-moi (chords rapides ~1.5s/accord) - une
fenetre de 2s peut chevaucher deux accords differents, et l'agregation
par minimum favorise alors artificiellement la note commune aux deux
plutot que le vrai accord. Fenetre plus courte = moins de ce risque.
Cette meme faiblesse existe probablement cote Python aussi (jamais
testee sur un morceau a changements d'accords aussi rapides) - a garder
en tete.

Non porte en JS pour l'instant (restent Python uniquement, dans
tools/chord-recognition/) : recognize_viterbi(), recognize_key_viterbi()
(lissage Viterbi complet) - le port JS actuel n'a que le modele de
tonalite simple, pas le lissage par transitions. Amelioration possible
future si besoin de meilleure precision.
